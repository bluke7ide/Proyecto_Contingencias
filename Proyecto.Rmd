  ...
  for (i in 1:500){
    ...
    for (j in 1:length(x[[1]])) {
      ...
      if (j + x[[3]] <= 65){
        ...
        VP_agui <- m_p/2*(1-(1/1.07))/(1.07^(1/2) - 1)
        VP <- VP_ann + VP_agui
...
VP <- (300000*(1-(1/1.07)))/(1.07^(1/12) - 1) + 150000*(1-(1/1.07))/(1.07^(1/2) - 1)
```
...
```{r}
datos_sim <- read_csv("simulaciones.csv", show_col_types = FALSE) %>% select(-`...1`)
```
...
ggplot() +
  ...
  ylim(0,5000000)
```
Nótesen las diferentes curvas visibles, como en la división de 47-48 en el percentil 50 y el quiebre en 42 en el percentil 90.
...
No lograría explicar el comportamiento quebrado en la simulación, más que un posible error en los cálculos cometidos, o algún comportamiento extraño casuístico que suceda cuando se trabaje con estas edades, parece ser 58 y 59. Lo más extraño es que en el percentil 90 no se observa el comportamiento, significando que es un problema local del percentil 50. Puede ser una edad selecta, pero se vería afectado en la prima regular, inclusive alguna coincidencia con la estructura de la semilla por default, pero todas suenan muy improbables. Tal vez los quiebres existen en todas partes, solo que se nota más en este caso particular. 
