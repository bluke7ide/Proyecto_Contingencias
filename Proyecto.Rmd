...
```{r}
anualidades <- primas(qx)$anualidad
edades <- numeric(500)
sexo <- numeric(500)
for (c in 1:500){
  edades[c] <- qx[[c]][[3]]
...
simulacion <- function(qx){
  ...
  df <- data.frame(edades, sexo, prima = suma_vp/anualidades)
  df <- df %>% 
    arrange(edades, sexo) %>%
    group_by(edades, sexo) %>%
    summarise(primas = mean(prima), .groups = "drop") %>% 
    select(primas)
  return(df[[1]])
}
```
...
# for(i in 1:100000){
#   print(i)
#   simulaciones[[i]] <- simulacion(qx)
# }
# proc.time() - t
```

```{r}
# exportar <- data.frame(primas_anuales[,1:2], simulaciones)
# names(exportar) <- c("edad", "sexo", 1:100000)
# write.csv(exportar,"simulaciones.csv")
```
